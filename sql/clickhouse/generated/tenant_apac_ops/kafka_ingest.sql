...
--
-- Placeholders:
--   tenant_apac_ops      - Tenant identifier (used ONLY in DWD section; omit for CDC DIM)
...
-- Usage: sed "s/\tenant_apac_ops/tenant_northwind/g; s/\${source_topic}/.../g; ..." kafka_ingest.sql.tpl | clickhouse-client -mn
-- ============================================================
...
--   tenant_apac_ops      = tenant_northwind
--   ${source_topic}   = tenant_apac_ops.staging_apm__sensor_readings.v1
--   ${target_table}   = dwd_apm_realtime_signal_readings
--   ${consumer_group} = ch_tenant_apac_ops_dwd_realtime_signal_readings

CREATE TABLE IF NOT EXISTS kafka_tenant_apac_ops_dwd_realtime_signal_readings
(
    event_id             String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tenant_apac_ops_dwd_realtime_signal_readings
TO ${target_table}
AS
SELECT
    event_id,
...
FROM kafka_tenant_apac_ops_dwd_realtime_signal_readings;

-- 2b. {tenant_id}.mart_apm__fct_anomaly_events.v1 → dwd_apm_anomaly_events
--   tenant_apac_ops      = tenant_northwind
--   ${source_topic}   = tenant_apac_ops.mart_apm__fct_anomaly_events.v1
--   ${target_table}   = dwd_apm_anomaly_events
--   ${consumer_group} = ch_tenant_apac_ops_dwd_anomaly_events

CREATE TABLE IF NOT EXISTS kafka_tenant_apac_ops_dwd_anomaly_events
(
    anomaly_id           String,
    schema_version       String,
    rule_id              String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tenant_apac_ops_dwd_anomaly_events
TO ${target_table}
AS
SELECT
    anomaly_id,
    ...
    source_event_ids,
    '${source_topic}'                                                  AS source_topic
FROM kafka_tenant_apac_ops_dwd_anomaly_events;

-- 2c. {tenant_id}.mart_apm__fct_late_sensor_readings.v1 → dwd_apm_late_sensor_readings
--   tenant_apac_ops      = tenant_northwind
--   ${source_topic}   = tenant_apac_ops.mart_apm__fct_late_sensor_readings.v1
--   ${target_table}   = dwd_apm_late_sensor_readings
--   ${consumer_group} = ch_tenant_apac_ops_dwd_late_sensor_readings

CREATE TABLE IF NOT EXISTS kafka_tenant_apac_ops_dwd_late_sensor_readings
(
    event_id        String,
    schema_version  String,
    tenant_id       String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tenant_apac_ops_dwd_late_sensor_readings
TO ${target_table}
AS
SELECT
    event_id,
    ...
    quality_flags,
    '${source_topic}'                                                  AS source_topic
FROM kafka_tenant_apac_ops_dwd_late_sensor_readings;

-- 2d. {tenant_id}.mart_apm__fct_stream_health_snapshots.v1 → dwd_apm_stream_health_snapshots
--   tenant_apac_ops      = tenant_northwind
--   ${source_topic}   = tenant_apac_ops.mart_apm__fct_stream_health_snapshots.v1
--   ${target_table}   = dwd_apm_stream_health_snapshots
--   ${consumer_group} = ch_tenant_apac_ops_dwd_stream_health_snapshots

CREATE TABLE IF NOT EXISTS kafka_tenant_apac_ops_dwd_stream_health_snapshots
(
    schema_version       String,
    job_name             String,
    tenant_id            String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tenant_apac_ops_dwd_stream_health_snapshots
TO ${target_table}
AS
SELECT
    schema_version,
    ...
    observed_at,
    '${source_topic}'                                                  AS source_topic
FROM kafka_tenant_apac_ops_dwd_stream_health_snapshots;

-- 2e. {tenant_id}.mart_apm__fct_realtime_diagnosis_events.v1 → dwd_apm_realtime_diagnosis_events
--   tenant_apac_ops      = tenant_northwind
--   ${source_topic}   = tenant_apac_ops.mart_apm__fct_realtime_diagnosis_events.v1
--   ${target_table}   = dwd_apm_realtime_diagnosis_events
--   ${consumer_group} = ch_tenant_apac_ops_dwd_realtime_diagnosis_events

CREATE TABLE IF NOT EXISTS kafka_tenant_apac_ops_dwd_realtime_diagnosis_events
(
    diagnosis_id        String,
    tenant_id           String,
    anomaly_id          String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tenant_apac_ops_dwd_realtime_diagnosis_events
TO ${target_table}
AS
SELECT
    diagnosis_id,
    ...
    created_at,
    '${source_topic}'                                                  AS source_topic
FROM kafka_tenant_apac_ops_dwd_realtime_diagnosis_events;

-- 2f. {tenant_id}.raw_apm__dlq_events.v1 → dwd_apm_dlq_events
--   tenant_apac_ops      = tenant_northwind
--   ${source_topic}   = tenant_apac_ops.raw_apm__dlq_events.v1
--   ${target_table}   = dwd_apm_dlq_events
--   ${consumer_group} = ch_tenant_apac_ops_dwd_dlq_events

CREATE TABLE IF NOT EXISTS kafka_tenant_apac_ops_dwd_dlq_events
(
    dlq_event_id   String,
    tenant_id      String,
    event_time     String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_tenant_apac_ops_dwd_dlq_events
TO ${target_table}
AS
SELECT
    dlq_event_id,
    ...
    error_message,
    '${source_topic}'                                                  AS source_topic
FROM kafka_tenant_apac_ops_dwd_dlq_events;
