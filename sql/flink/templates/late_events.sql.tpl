...
SET 'pipeline.name' = '{{tenant_id}}-late_events';

CREATE TABLE {{tenant_id}}_late_events_sensor_readings (
  ...
  'properties.group.id' = '{{tenant_id}}-late-events',
  'scan.startup.mode' = 'earliest-offset',
  'format' = 'json',
  'json.ignore-parse-errors' = 'true'
);

CREATE TABLE {{tenant_id}}_late_events_sink (
  event_id STRING,
  ...
  'connector' = 'kafka',
  'topic' = '{{tenant_id}}.mart_apm__fct_late_sensor_readings.v1',
  'properties.bootstrap.servers' = 'redpanda:19092',
  'format' = 'json'
);

INSERT INTO {{tenant_id}}_late_events_sink
SELECT
  event_id,
  'late_event.v1',
  tenant_id,
  ...
  END,
  quality_flags
FROM {{tenant_id}}_late_events_sensor_readings
