...
SET 'pipeline.name' = '{{tenant_id}}-enrich';

CREATE TABLE {{tenant_id}}_enrich_raw_sensor_events (
  ...
  'properties.group.id' = '{{tenant_id}}-enrich',
  'scan.startup.mode' = 'earliest-offset',
  'format' = 'json'
);

CREATE TABLE {{tenant_id}}_enrich_cdc_tags (
  ...
  'properties.bootstrap.servers' = 'redpanda:19092',
  'properties.group.id' = '{{tenant_id}}-enrich-cdc-tags',
  'scan.startup.mode' = 'earliest-offset',
  'format' = 'json'
);

CREATE TABLE {{tenant_id}}_enrich_cdc_assets (
  ...
  'properties.bootstrap.servers' = 'redpanda:19092',
  'properties.group.id' = '{{tenant_id}}-enrich-cdc-assets',
  'scan.startup.mode' = 'earliest-offset',
  'format' = 'json'
);

CREATE TABLE {{tenant_id}}_enrich_cdc_comps (
  ...
  'properties.bootstrap.servers' = 'redpanda:19092',
  'properties.group.id' = '{{tenant_id}}-enrich-cdc-comps',
  'scan.startup.mode' = 'earliest-offset',
  'format' = 'json'
);

CREATE TABLE {{tenant_id}}_enrich_staging_enriched (
...
INSERT INTO {{tenant_id}}_enrich_staging_enriched
...
FROM {{tenant_id}}_enrich_raw_sensor_events r
LEFT JOIN {{tenant_id}}_enrich_cdc_tags m
  ON r.tenant_id = m.tenant_id AND r.tag_id = m.tag_id
LEFT JOIN {{tenant_id}}_enrich_cdc_assets a
  ON m.tenant_id = a.tenant_id AND m.asset_id = a.asset_id
LEFT JOIN {{tenant_id}}_enrich_cdc_comps c
